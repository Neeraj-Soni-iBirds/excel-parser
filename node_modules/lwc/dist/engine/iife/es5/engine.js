var LWC = (function (exports) {
    ...
    ***************************************************************************** */
     ...
     * For full license text, see the LICENSE file in the repo root or https://opensource.org/licenses/MIT
     */
    var _c = Element.prototype, hasAttribute = _c.hasAttribute, getAttribute = _c.getAttribute, setAttribute = _c.setAttribute, removeAttribute = _c.removeAttribute;
    /*
     * Copyright (c) 2018, salesforce.com, inc.
     * All rights reserved.
     * SPDX-License-Identifier: MIT
    ...
    var _e = Array.prototype, forEach = _e.forEach, ArrayIndexOf = _e.indexOf, ArrayJoin = _e.join, ArrayMap = _e.map, ArrayPush = _e.push, ArrayReduce = _e.reduce, ArraySlice$1 = _e.slice, ArrayUnshift$1 = _e.unshift;
    ...
    function getPropNameFromAttrName(attrName) {
        if (isUndefined(AttrNameToPropNameMap[attrName])) {
            AttrNameToPropNameMap[attrName] = StringReplace$1.call(attrName, CAMEL_REGEX, function (g) { return g[1].toUpperCase(); });
        }
    ...
    */
    ...
    function getLightningElementPrototypeRestrictionsDescriptors(proto) {
        ...
        var originalIsConnectedGetter = getOwnPropertyDescriptor(proto, 'isConnected').get;
        var descriptors = {
            ...
            isConnected: generateAccessorDescriptor({
                get: function () {
                    var vm = getAssociatedVM(this);
                    var componentTag = getComponentTag(vm);
    ...
    function register(service) {
        ...
        for (var i_4 = 0; i_4 < hooks.length; ++i_4) {
            var hookName = hooks[i_4];
    ...
    function invokeServiceHook(vm, cbs) {
        ...
        for (var i_5 = 0, len = cbs.length; i_5 < len; ++i_5) {
            cbs[i_5].call(undefined, component, data, def, context);
        }
    }
    /*
     * Copyright (c) 2018, salesforce.com, inc.
    ...
    function ti(value) {
        ...
        if (process.env.NODE_ENV !== 'production') {
            ...
            if (shouldNormalize) {
                logError("Invalid tabindex value `" + toString(value) + "` in template for " + vmBeingRendered_1 + ". This attribute must be set to 0 or -1.", vmBeingRendered_1);
            }
        }
    ...
    function validateSlots(vm, html) {
        ...
        var _b = vm.cmpSlots, cmpSlots = _b === void 0 ? EmptySlots : _b;
        var _c = html.slots, slots = _c === void 0 ? EmptyArray : _c;
    ...
    function invokeComponentRenderedCallback(vm) {
        ...
        if (!isUndefined(renderedCallback)) {
            var vmInvokingRenderedCallbackInception_1 = vmInvokingRenderedCallback;
            runWithBoundaryProtection(vm, owner, function () {
                ...
                vmInvokingRenderedCallback = vmInvokingRenderedCallbackInception_1;
            });
        }
    }
    ...
    var _g = Array.prototype, ArraySplice$1 = _g.splice, ArrayIndexOf$1 = _g.indexOf, ArrayPush$1 = _g.push;
    ...
    function valueMutated(target, key) {
        ...
        if (!isUndefined$1(reactiveRecord)) {
            ...
            if (!isUndefined$1(reactiveObservers)) {
                for (var i_6 = 0, len = reactiveObservers.length; i_6 < len; i_6 += 1) {
                    var ro = reactiveObservers[i_6];
                    ro.notify();
                }
            }
        }
    ...
    var ReactiveObserver = /** @class */ (function () {
        ...
        ReactiveObserver.prototype.reset = function () {
            ...
            if (len > 0) {
                for (var i_7 = 0; i_7 < len; i_7 += 1) {
                    var set = listeners[i_7];
                    var pos = ArrayIndexOf$1.call(listeners[i_7], this);
    ...
    function markComponentAsDirty(vm) {
        if (process.env.NODE_ENV !== 'production') {
            ...
            assert.isFalse(isInvokingRender, "markComponentAsDirty() for " + vm + " cannot be called during rendering of " + vmBeingRendered_2 + ".");
            assert.isFalse(isUpdatingTemplate, "markComponentAsDirty() for " + vm + " cannot be called while updating template of " + vmBeingRendered_2 + ".");
        }
        vm.isDirty = true;
    }
        ...
        return {
            ...
            set: function (newValue) {
                ...
                if (process.env.NODE_ENV !== 'production') {
                    ...
                    assert.invariant(!isInvokingRender, vmBeingRendered_3 + ".render() method has side effects on the state of " + vm + "." + propName);
                    assert.invariant(!isUpdatingTemplate, "When updating the template of " + vmBeingRendered_3 + ", one of the accessors used by the template has side effects on the state of " + vm + "." + propName);
    ...
    BaseLightningElementConstructor.prototype = {
        ...
        addEventListener: function (type, listener, options) {
            ...
            if (process.env.NODE_ENV !== 'production') {
                ...
                assert.invariant(!isInvokingRender, vmBeingRendered_4 + ".render() method has side effects on the state of " + vm + " by adding an event listener for \"" + type + "\".");
                assert.invariant(!isUpdatingTemplate, "Updating the template of " + vmBeingRendered_4 + " has side effects on the state of " + vm + " by adding an event listener for \"" + type + "\".");
    ...
    function HTMLBridgeElementFactory(SuperClass, props, methods) {
        ...
        for (var i_8 = 0, len = props.length; i_8 < len; i_8 += 1) {
            var propName = props[i_8];
        ...
        for (var i_9 = 0, len = methods.length; i_9 < len; i_9 += 1) {
            var methodName = methods[i_9];
        ...
        return {
            ...
            set: function (newValue) {
                ...
                if (process.env.NODE_ENV !== 'production') {
                    ...
                    assert.invariant(!isInvokingRender, vmBeingRendered_5 + ".render() method has side effects on the state of " + vm + "." + toString(key));
                    assert.invariant(!isUpdatingTemplate, "Updating the template of " + vmBeingRendered_5 + " has side effects on the state of " + vm + "." + toString(key));
                }
        ...
        return {
            ...
            set: function (newValue) {
                ...
                if (process.env.NODE_ENV !== 'production') {
                    ...
                    assert.invariant(!isInvokingRender, vmBeingRendered_6 + ".render() method has side effects on the state of " + vm + "." + toString(key));
                    assert.invariant(!isUpdatingTemplate, "Updating the template of " + vmBeingRendered_6 + " has side effects on the state of " + vm + "." + toString(key));
                }
        ...
        return {
            ...
            set: function (newValue) {
                ...
                if (process.env.NODE_ENV !== 'production') {
                    ...
                    assert.invariant(!isInvokingRender, vmBeingRendered_7 + ".render() method has side effects on the state of " + vm + "." + toString(key));
                    assert.invariant(!isUpdatingTemplate, "Updating the template of " + vmBeingRendered_7 + " has side effects on the state of " + vm + "." + toString(key));
                }
                if (set) {
    ...
    function getCtorProto(Ctor, subclassComponentName) {
        ...
        if (isCircularModuleDependency(proto)) {
            var p_1 = resolveCircularModuleDependency(proto);
            if (process.env.NODE_ENV !== 'production') {
                if (isNull(p_1)) {
            ...
            // be base.
            proto = p_1 === proto ? BaseLightningElement : p_1;
        }
        return proto;
    }
    ...
    function flushRehydrationQueue() {
        ...
        for (var i_11 = 0, len = vms.length; i_11 < len; i_11 += 1) {
            var vm = vms[i_11];
            try {
                rehydrate(vm);
            }
                ...
                if (i_11 + 1 < len) {
                    ...
                    ArrayUnshift$1.apply(rehydrateQueue, ArraySlice$1.call(vms, i_11 + 1));
    ...
    function runShadowChildNodesDisconnectedCallback(vm) {
        ...
        for (var i_12 = vCustomElementCollection.length - 1; i_12 >= 0; i_12 -= 1) {
            var elm = vCustomElementCollection[i_12].elm; // There are two cases where the element could be undefined:
    ...
    function recursivelyDisconnectChildren(vnodes) {
        for (var i_13 = 0, len = vnodes.length; i_13 < len; i_13 += 1) {
            var vnode = vnodes[i_13];
    ...
    function allocateInSlot(vm, children) {
        ...
        for (var i_14 = 0, len = children.length; i_14 < len; i_14 += 1) {
            var vnode = children[i_14];
            if (isNull(vnode)) {
                continue;
            }
            var data = vnode.data;
        ...
        if (isFalse$1(vm.isDirty)) {
            ...
            for (var i_15 = 0, len = oldKeys.length; i_15 < len; i_15 += 1) {
                var key = oldKeys[i_15];
     ...
     * For full license text, see the LICENSE file in the repo root or https://opensource.org/licenses/MIT
     */
    var _j = Node.prototype, appendChild = _j.appendChild, insertBefore = _j.insertBefore, removeChild = _j.removeChild, replaceChild = _j.replaceChild;
    /*
     * Copyright (c) 2018, salesforce.com, inc.
     * All rights reserved.
     * SPDX-License-Identifier: MIT
    ...
    function buildCustomElementConstructor(Ctor, options) {
        ...
        return _a = /** @class */ (function (_super) {
            ...
            _a.observedAttributes = ArrayMap.call(getOwnPropertyNames(props), function (propName) { return props[propName].attr; }), _a;
    }
    /** version: 1.3.2 */
